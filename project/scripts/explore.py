...
class Explore:
    ...
    def __init__(self, grid):
        ...
        self.range = int(0.3/self.grid.resolution)
        self.tr = Transform()
    ...
    def exploration_score(self, current_index, rand_index):
        index_range = int(self.range/self.grid.resolution)
        x_max = rand_index[0] + index_range
        if x_max > self.grid.dim[0]:
    ...
    def next_goal(self, current_pose):
        ...
        current_index = self.grid.convert_to_index(current_position)
        found_point = False
        max_index = 0
        count = 0
        ...
        if found_point:
            ...
            next_pose = self.tr.pose_stamped(position, yaw)
            return next_pose
        else:
            return None
